    ...
    public partial class Form1 : Form
    {
        ...
        private void _serverManager_Received(Request request)
        {
            ...
            switch (request.Path)
            {
                ...
                case "message":
                    sendReq.Path = "message";
                    auditMessage = $"{Encoding.UTF8.GetString(request.Nickname)}: {Encoding.UTF8.GetString(request.Data)}";
                    sendReq.Data = Encoding.UTF8.GetBytes(auditMessage);
                    break;
                case "file":
                    byte[] file = request.Data;
                    File.WriteAllBytes("test.txt", file);
                    auditMessage = $"{Encoding.UTF8.GetString(request.Nickname)}: test.txt was downloaded";
                    sendReq.Data = Encoding.UTF8.GetBytes(auditMessage);
                    break;
            }
